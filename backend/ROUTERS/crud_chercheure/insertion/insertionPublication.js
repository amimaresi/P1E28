...
const insertionPublication = async (req, res ) => {
    ...
    const { Date  ,confJourn , volume, MaisonEdistion , pages, rang, Titre, Lien, Membres, Classement } = req.body
    const Chercheur = req.params.id
    if (!Date || !Chercheur || !confJourn ||!MaisonEdistion|| !volume || !pages || !rang || !Titre || !Lien || !Membres || !Classement){
   ...
   const res = await publication.save()
